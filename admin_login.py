...
image = Image.open("Images/—Pngtree—flat wind blue doctor reception_5861560.png")  # Replace with your actual image file
...
def login():
    username = admin_entry.get().strip()
    password = admin_pass.get().strip()
    security = admin_sec.get().strip()

    with sqlite3.connect('adminDb.db') as db:
        c = db.cursor()
        c.execute("SELECT user_name FROM admin WHERE user_name = ? AND password = ? AND security_code = ?", (username, password, security))
        result = c.fetchone()

        if result:
            ms.showinfo("Message", "Login successful")
            
            call(["python", "admin_panel.py"])  # Pass username
        else:
            ms.showerror("Oops!", "Username or Password or Security Code is incorrect.")
...
admin_entry = ctk.CTkEntry(button_frame, placeholder_text="Username", width=300, height=40)
admin_entry.pack(pady=12, padx=10)

admin_pass = ctk.CTkEntry(button_frame, placeholder_text="Password", show="*", width=300, height=40)
admin_pass.pack(pady=12, padx=10)

admin_sec = ctk.CTkEntry(button_frame, placeholder_text="Security Code", show="*", width=300, height=40)
admin_sec.pack(pady=12, padx=10)
...
def register():
    call(["python", "admin_register.py"])

register_label = ctk.CTkLabel(button_frame, text="Not an Admin? Register here..", text_color="#007acc", cursor="hand2")
